...
export const DelayDrawer: React.FC<DelayDrawerProps> = ({ isOpen, onClose, variant = 'drawer' }) => {
      ...
      {isOpen && (
        <>
          ...
          {variant === 'drawer' ? (
            ...
            >
              <div className="glass-card w-full max-w-lg p-6 text-zinc-900 dark:text-zinc-100 transition-colors max-h-[85vh] overflow-y-auto">
                {!isDelaying ? (
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <h2 className="text-xl font-semibold">我想分心了</h2>
                      <button onClick={onClose} className="p-2 rounded-full bg-zinc-100/60 dark:bg-zinc-800/60 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors">
                        <X size={20} />
                      </button>
                    </div>
                    <div>
                      ...
                      <input 
                        ...
                        value={intensity}
                        onChange={e => setIntensity(Number(e.target.value))}
                        className="w-full accent-blue-500 bg-zinc-200/70 dark:bg-zinc-800/70 rounded-lg appearance-none h-2 transition-colors"
                      />
                      <div className="flex justify-between text-xs text-zinc-400 dark:text-zinc-500 mt-1 transition-colors">
                        <span>轻微</span>
                        <span>难以忍受</span>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[30, 120, 300].map(time => (
                          <button
                            ...
                            onClick={() => setDelayLevel(time as any)}
                            className={cn(
                              "py-3 rounded-xl border transition-colors backdrop-blur-md",
                              delayLevel === time 
                                ? "bg-blue-600/90 border-blue-500/60 text-white" 
                                : "bg-white/40 dark:bg-black/20 border-white/40 dark:border-white/10 text-zinc-700 dark:text-zinc-200 hover:bg-white/60 dark:hover:bg-white/10"
                            )}
                          >
                            {time < 60 ? `${time}秒` : `${time/60}分钟`}
                          </button>
                    ...
                    <button 
                      onClick={handleStartDelay}
                      className="w-full py-4 rounded-2xl bg-zinc-900 dark:bg-white text-white dark:text-black font-semibold text-lg hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors mt-4"
                    >
                      开始时延
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col items-center py-8 space-y-8">
                    ...
                    {timeLeft === 0 && (
                      <div className="grid grid-cols-2 gap-4 w-full mt-8">
                        <button 
                          onClick={() => handleOutcome('yielded')}
                          className="py-4 rounded-2xl bg-white/40 dark:bg-black/20 border border-white/40 dark:border-white/10 text-zinc-700 dark:text-zinc-200 font-medium hover:bg-white/60 dark:hover:bg-white/10 transition-colors"
                        >
                          还是想分心 (断链)
                        </button>
                        <button 
                          onClick={() => handleOutcome('overcome')}
                          className="py-4 rounded-2xl bg-blue-600/90 text-white font-medium hover:bg-blue-500 shadow-lg shadow-blue-900/20 transition-colors"
                        >
                          扛过去了 (继续)
                        </button>
                      </div>
